...
import { UserPlus, User, KeyRound, AlertCircle, CheckCircle2, RefreshCw } from 'lucide-react';
...
const Register = ({ onToggleLogin }) => {
  ...
  const { loading, error: apiError, execute, resetError } = useApi();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await execute(() => authService.register(username, password, role));
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 transition-colors duration-300">
      <div className="w-full max-w-md glass-card p-8 border border-slate-200/80 dark:border-gray-800/80 bg-white/70 dark:bg-[#111827]/50 shadow-2xl relative overflow-hidden rounded-3xl">
        {/* Glow decorators */}
        <div className="absolute -top-12 -right-12 w-32 h-32 bg-indigo-500/10 rounded-full blur-2xl animate-pulse-slow animate-pulse-slow"></div>
        ...
        <div className="text-center mb-8 relative">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight font-display">Register Account</h2>
          <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 uppercase font-bold tracking-widest">Join the IPL Cric-AI platform</p>
        </div>
        ...
        {success && (
          <div className="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-sm flex items-center space-x-2 animate-pulse">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Confirm Password */}
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider block">Confirm Password</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center text-gray-400 dark:text-gray-500">
                <KeyRound className="w-4 h-4" />
              </span>
          ...
          {/* Role Select */}
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider block">Account Role</label>
            <select
              value={role}
            ...
            >
              <option value="user">Standard User (Make Predictions)</option>
              <option value="admin">Administrator (Run ML Retraining)</option>
            </select>
          </div>
